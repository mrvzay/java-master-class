...
public class Store {
    ...
    private void mangeStoreCarts() {
        ...
        cart2.addItem(inventory.get("B100"), 20);
    ...
    private void stockAisles() {
        ...
        for (InventoryItem item : inventory.values()) {
            ...
            Map<String, InventoryItem> produceMap = aisleInventory.get(aisle);
            if (produceMap == null) {
                produceMap = new TreeMap<>();
            }
            produceMap.put(item.getProduct().name(), item);
            aisleInventory.putIfAbsent(aisle, produceMap);
        }
    }
